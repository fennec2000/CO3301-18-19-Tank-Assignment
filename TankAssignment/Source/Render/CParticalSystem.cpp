...
void CParticalSystem::Render()
{
	...
	GS_Consts->ViewProjMatrix = MainCamera->GetViewProjectionD3DXMatrix();
	D3DXMATRIX viewMatrix = MainCamera->GetViewD3DXMatrix();
...
void CParticalSystem::Update(TFloat32 updateTime)
{
	...
	GS_ConstBuffer->Map(D3D10_MAP_WRITE_DISCARD, NULL, (void**)&GS_Consts);
	GS_Consts->UpdateTime = (float)updateTime;
	GS_ConstBuffer->Unmap();
